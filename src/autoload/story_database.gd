...
func _create_chapters():
	...
	ch.unlock_level = 1
	ch.map_areas = ["杭州", "杭州郊外", "西湖"]
	ch.main_quest = "quest_ch1_main"
	ch.side_quests = ["quest_ch1_side_1", "quest_ch1_side_2"]
	ch.boss_battle = "battle_ch1_boss"
	ch.rewards = {"exp": 5000, "copper": 10000, "items": ["wooden_sword", "cloth_clothes"]}
	ch.choices = ["choice_ch1_join_sector", "choice_ch1_help_beggar"]
	...
	ch.unlock_level = 10
	ch.map_areas = ["杭州", "苏州", "太湖", "虎丘"]
	ch.main_quest = "quest_ch2_main"
	ch.side_quests = ["quest_ch2_side_1", "quest_ch2_side_2", "quest_ch2_side_3"]
	ch.boss_battle = "battle_ch2_boss"
	ch.rewards = {"exp": 15000, "copper": 30000, "items": ["iron_sword", "leather_armor"]}
	ch.choices = ["choice_ch2_sect_choice", "choice_ch2_save_girl"]
	story_chapters[ch.id] = ch
	...
	ch.unlock_level = 20
	ch.map_areas = ["恒山", "华山", "昆仑山", "洛阳", "铁石岛"]
	ch.main_quest = "quest_ch3_main"
	ch.side_quests = ["quest_ch3_side_1", "quest_ch3_side_2", "quest_ch3_side_3", "quest_ch3_side_4"]
	ch.boss_battle = "battle_ch3_boss"
	ch.rewards = {"exp": 30000, "copper": 50000, "items": ["sect_weapon", "sect_xinfa"]}
	ch.choices = ["choice_ch3_sect_loyalty", "choice_ch3_rival"]
	story_chapters[ch.id] = ch
	...
	ch.unlock_level = 30
	ch.map_areas = ["京城", "少林寺", "武当山", "峨眉山", "五岳"]
	ch.main_quest = "quest_ch4_main"
	ch.side_quests = ["quest_ch4_side_1", "quest_ch4_side_2", "quest_ch4_side_3"]
	ch.boss_battle = "battle_ch4_boss"
	ch.rewards = {"exp": 50000, "copper": 100000, "items": ["famous_sword", "rare_xinfa"]}
	ch.choices = ["choice_ch4_love", "choice_ch4_righteousness"]
	story_chapters[ch.id] = ch
...
func _get_chapter_choices(idx: int) -> Array[String]:
	var choices = {
		5: ["choice_ch5_country", "choice_ch5_friend"],
		6: ["choice_ch6_sword_owner", "choice_ch6_sword_secret"],
		7: ["choice_ch7_demon_spy", "choice_ch7_purge"],
		8: ["choice_ch8_champion", "choice_ch8_withdraw"],
		9: ["choice_ch9_truth", "choice_ch9_sacrifice"],
		10: ["choice_ch10_new_beginning", "choice_ch10_legacy"],
		11: ["choice_ch11_wanderer", "choice_ch11_settle"],
		12: ["choice_ch12_end_jianghu", "choice_ch12_end_retire"]
	}
...
func _create_side_quests():
	...
	for i in range(1, 13):
		for j in range(1, 4):
			...
			q.choices = ["choice_side_%d_%d_a" % [i, j], "choice_side_%d_%d_b" % [i, j]]
			quests[q.id] = q
...
func _create_companion_quests():
	...
	for char in companions:
		if char.recruit_chapter > 0:
			...
			q.type = "侠客招募"
			q.objectives = [{"id": "recruit_%s" % char.id, "type": "对话", "description": "完成%s的招募条件" % char.name, "target": char.id, "count": 1}]
...
func _create_sect_quests():
	for sect_id in SectDatabase.get_instance().get_all_sects():
		...
		for level in [1, 5, 10, 15, 20]:
			...
			q.chapter = 3
			q.type = "门派日常"
			q.objectives = [{"id": "sect_daily", "type": "击杀", "description": "完成门派日常", "target": "sect_enemy", "count": 5}]
...
func _create_guild_quests():
	...
	for t in types:
		for level in [1, 2, 3]:
			...
			q.type = "帮会任务"
			q.objectives = [{"id": "guild_%s" % t, "type": t, "description": "完成帮会%s" % t, "target": "guild", "count": level * 10}]
...
func _create_encounter_quests():
	...
	for e in encounters:
		...
		q.objectives = [{"id": "encounter_complete", "type": "完成奇遇", "description": "完成奇遇剧情", "target": "encounter", "count": 1}]
	...
	for i in range(1, 13):
		...
		if chapter:
			for j in range(3):
				...
				d.choices = ["choice_continue", "choice_ask_detail"]
	...
	for npc in npcs.values():
		if npc.is_city_npc or npc.is_sect_npc:
			for j in range(5):
				...
				d.choices = ["choice_chat", "choice_gift", "choice_leave"]
				d.affection_change = {"chat": 1, "gift": 5}
				dialogues[d.id] = d
...
func _create_recruit_dialogues():
	for char in CharacterDatabase.get_instance().get_recruitable_characters():
		...
		d.choices = ["choice_recruit_accept", "choice_recruit_refuse", "choice_recruit_later"]
...
func _create_encounter_dialogues():
	for q in quests.values():
		if q.type == "奇遇":
			...
			d.choices = ["choice_encounter_accept", "choice_encounter_refuse"]
			d.rewards = q.rewards
			dialogues[d.id] = d
